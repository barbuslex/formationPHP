...
if(isset($_GET['id']) && is_numeric($_GET['id'])) {
    if($_POST) {
        if(!$_POST['username']) {
            ...
            header('Location: index.php?p=utilisateur_modifier');
            exit();
        }
...
<body>
    <div class="container">
        ...
        <form method="post" action="index.php?p=utilisateur_modifier&id=<?= $_GET['id']; ?>">
            <div class="mb-3">
                <label for="username" class="form-label">Nom d'utilisateur :</label>
                <input id="username" name="username" type="text" class="form-control" value="<?= $utilisateur['username'] ?>">
            </div>
            <div class="mb-3 text-end">
                <a href="index.php?p=utilisateur_liste" class="btn btn-secondary">Annuler</a>
